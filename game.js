...
BasicGame.Game.prototype = {
  ...
  preload:function(){
    this.load.image('sea', 'assets/purple.png');
    this.load.image('bullet', 'assets/bullet.png');
    this.load.image('enemyBullet', 'assets/laser.png');  
    this.load.image('powerup1', 'assets/powerup1.png');
    this.load.image('meteor', 'assets/meteor.png', 64, 64);
  ...
  setupMeteors: function(){
    this.meteorPool = this.add.group();
    this.meteorPool.enableBody = true;
    this.meteorPool.physicsBodyType = Phaser.Physics.ARCADE;
    this.meteorPool.createMultiple(50, 'meteor');
    this.meteorPool.setAll('anchor.x', 0.5);
    this.meteorPool.setAll('anchor.y', 0.5);
    this.meteorPool.setAll('outOfBoundsKill', true);
    this.meteorPool.setAll('checkWorldBounds', true);
    this.nextMeteorAt = 0;
    this.meteorDelay = 10000;
    this.meteorInitialHealth = 7;
  ...
  checkCollisions:function(){
    ...
    this.physics.arcade.overlap(
      this.bulletPool, this.meteorPool, this.enemyHit, null, this
    );
    ...
    this.physics.arcade.overlap(
      this.player, this.meteorPool, this.playerHit, null, this
    );
  ...
  spawnMeteors: function(){
    if (this.nextMeteorAt < this.time.now && this.meteorPool.countDead() > 0) {
      this.nextMeteorAt = this.time.now + this.meteorDelay;
      var meteor = this.meteorPool.getFirstExists(false);
      // spawn at a random location at the top
      meteor.reset(this.rnd.integerInRange(20, 1004), 0,
      this.meteorInitialHealth);
      // choose a random target location at the bottom
      var target = this.rnd.integerInRange(20, 1004);
      // move to target and rotate the sprite accordingly
      meteor.rotation = this.physics.arcade.moveToXY(
        meteor, target, 768, this.rnd.integerInRange(30, 80)
      ) - Math.PI / 2;
    }
  ...
  damageEnemeny:function(enemy, damage){
    enemy.damage(damage);
    if(enemy.alive){
      enemy.play('hit');
    } else {
  ...
  fire: function() {
    ...
    if (this.weaponLevel > 0) {
      ...
      for (var i = 0, wl = this.weaponLevel; i < wl; i++) {
        bullet = this.bulletPool.getFirstExists(false);
  ...
  enemyFire: function() {
    this.shooterPool.forEachAlive(function (enemy) {
      if (this.time.now > enemy.nextShotAt && this.enemyBulletPool.countDead() > 0) {
        ...
        bullet.rotation = this.physics.arcade.moveToObject(bullet, this.player, 150) - Math.PI / 2;
